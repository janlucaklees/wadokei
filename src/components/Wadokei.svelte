...
>
	{#each getTimedPeriods(dayPeriods, sunrise, sunset) as period (period.period.zodiacSign)}
		<SvgPeriod timedPeriod={period} {radius} />
	{/each}

	{#each getTimedPeriods(nightPeriods, sunset, addDays(sunrise, 1)) as period (period.period.zodiacSign)}
		<SvgPeriod timedPeriod={period} {radius} />
	{/each}
